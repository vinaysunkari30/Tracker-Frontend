...
class ProjectsPage extends Component{
	...
	getProjectsList = async() => {
		...
		const response = await fetch('https://tracker-backend-vg3b.onrender.com/projects', options);
				...
				<div className='container-fluid'>
					<div className='row'>
					{
						projectsList.length===0 ? (
							<div className='no-projects-div d-flex justify-content-center align-items-center'>
								<div className='d-flex flex-column justify-content-center p-4'>
									<h1 className='empty-projects-heading text-center'>No projects found. Create your first project!</h1>
									<div className='d-flex justify-content-center'>
										<button data-bs-toggle="modal" data-bs-target="#myModal" className='new-project-btn d-flex justify-content-center align-items-center mt-4'>
						...
						(<div className='filled-projects-page'>
							<h1 className='text-center projects-heading'>Your Projects</h1>
							<div className='d-flex justify-content-center'>
								<button data-bs-toggle="modal" data-bs-target="#myModal" className='new-project-btn d-flex justify-content-center align-items-center mt-4'>
									<FaPlus className='icon' />
									<h2 className='btn-text'>New Project</h2>
							...
							<ul className='d-flex flex-wrap list-unstyled p-0 mt-2'>
								{
									projectsList.map(eachSite => (
										<ProjectPage key={eachSite.id} siteData={eachSite} onDeleteProject={this.onDeleteProject}/>
									))
								}
							</ul>
						</div>)
				...
				<div class="modal-dialog modal-dialog-centered">
					<div class="modal-content p-3">
						<div className='d-flex justify-content-between'>
							<h2 class="modal-title" id="myModalLabel">New Project</h2>
						...
						<div class="modal-body d-flex flex-column">
							...
							<input className='modal-input-field' onChange={this.onGetProjectName} value={projectName} type='text' placeholder='Project Name' />
						</div>
						<div className='d-flex justify-content-end mt-1 pe-3 pb-2'>
							{isValid ? <button onClick={this.onCreateProject} type="button" class="create-btn" data-bs-dismiss="modal" aria-label="Close">Create</button>: ''}
						</div>
					</div>
				</div>
			</div>
